  ...
  const handleCommentEdit = async (cId: string, comment: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.message || "Some error occurred while updating", {
          id: toastId,
        });
      }
    } catch (error: unknown) {
      ...
      <CardHeader className="">
        <div className="flex items-center space-x-3">
          <Avatar>
            <AvatarImage
              src={
                post.author?.userDetails?.profilePhoto ||
        ...
        <div className="flex items-center mb-3 space-x-4">
          <div className="flex items-center">
            ...
            <span className="text-sm">
              {formatPrice(post.priceRangeStart)} -{" "}
              {formatPrice(post.priceRangeEnd)}
            </span>
          </div>
        </div>
        ...
        <div className="flex justify-between w-full text-sm text-gray-500 mb-2">
          <div className="flex h-5 items-center space-x-4 text-sm">
            <div className="flex justify-center items-center">
              {" "}
              <ArrowBigUp className="h-4 w-4" /> {upVoteCount}
            </div>
